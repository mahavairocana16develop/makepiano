...
- **ピアノビュー**：88 鍵の鍵盤と落下ノート表示（右手＝青、左手＝緑）。鍵盤の下に左右の手を描き、自動運指に従ってどの指がどの鍵を押すかを示します。落下ノートにも指番号を表示
...
- 運指は動的計画法で自動割り当てし（指の組み合わせごとの楽な広がり・同指の跳躍・親指の黒鍵などをコスト化）、楽譜に指番号として記譜します（和音は 3 音まで。`--no-fingering` で省略）。
